...
df = session.sql("""
    SELECT c.CLUSTER_ID, e.CONTENT
    FROM MCWICS_HACKATHON_DATA.PUBLIC.SCRAPEDATA_CAPTION_CLUSTERS c
    JOIN MCWICS_HACKATHON_DATA.PUBLIC.SCRAPEDATA_CAPTION_EMBEDS e
        ON c.CONTENT_HASH = e.CONTENT_HASH
...
insights_text = str(insights_df["INSIGHTS"].iloc[0])
forecast_text = str(insights_df["FORECAST_SUMMARY"].iloc[0])

insights_html = insights_text.replace('\n\n', '<br><br>').replace('\n', '<br>')
insights_html = re.sub(r'\*\*(.+?)\*\*', r'<strong>\1</strong>', insights_html)
insights_html = re.sub(r'### (\d+\.)', r'<br><br><strong>\1</strong>', insights_html)
insights_html = re.sub(r'\*   ', r'<br>• ', insights_html)
...
<div class="insight-card">
    ...
    <div>{forecast_text.replace(chr(10), '<br>')}</div>
</div>
""", unsafe_allow_html=True)
